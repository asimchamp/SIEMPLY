...
async def _ensure_tmux_installed(ssh) -> bool:
    """Ensure tmux is installed on the remote host (best-effort). Returns True if available."""
    check_tmux = await ssh.run("which tmux")
    if check_tmux.returncode == 0:
        return True

    # Try to install tmux; ignore failures
    install_cmds = [
...
async def _run_download_in_tmux(ssh, session_name: str, download_cmd: str, output_path: str, timeout_seconds: int = 1800) -> bool:
    """Run a download command in a remote tmux session and wait until the file appears or timeout."""
    # Create detached tmux session to run the download
    rc_file = f"/tmp/splunk_install/{session_name}.rc"
    wrapped = (
        f"bash -lc '{download_cmd}; echo $? | sudo tee {rc_file} >/dev/null'"
    )
    start = await ssh.run(f"tmux new-session -d -s {session_name} \"{wrapped}\"")
    if start.returncode != 0:
        logger.warning("Failed to start tmux session, falling back to inline download")
        return False
    logger.info("Started tmux session: %s", session_name)

    # Poll for output existence or rc file
    elapsed = 0
    interval = 5
    while elapsed < timeout_seconds:
        check = await ssh.run(f"test -s {output_path} && echo ok || echo no")
        if "ok" in check.stdout:
            return True
        rc_check = await ssh.run(f"test -f {rc_file} && cat {rc_file} || echo none")
        if rc_check.stdout.strip().isdigit():
            # Command finished but file missing -> failure
            return False
...
async def install_splunk_uf(host: Host, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async with get_ssh_client(host) as ssh:
        ...
        cluster_role = params.get("cluster_role")
            ...
            try:
                ...
                if cluster_manager.validate_cluster_exists(cluster_name):
                    ...
                    copy_result = await cluster_manager.copy_component_configs_direct(
                        ssh, cluster_name, component_type, f"{install_dir}/splunkforwarder/etc/system", host.ip_address
                    )
                    ...
                    if copy_result["success"]:
                        ...
                        fallback_result = await cluster_manager.copy_component_configs_to_host(
                            ssh, cluster_name, component_type, f"{install_dir}/splunkforwarder/etc/system"
                        )
        ...
        # Ensure tmux is available for long-running steps (best-effort)
        has_tmux = await _ensure_tmux_installed(ssh)
        ...
        try:
            ...
            # Try downloading from each URL until one succeeds (use tmux when available)
            download_success = False
            all_download_errors = []

            for url_index, download_url in enumerate(download_urls):
                session_name = f"siemply_dl_uf_{os.getpid()}_{url_index}"
                # Prefer curl
                if has_curl:
                    base_cmd = f"cd /tmp/splunk_install && sudo curl -L -o splunkforwarder.tgz '{download_url}' 2>&1"
                    if has_tmux:
                        logger.info(f"Attempting download with curl in tmux from URL #{url_index+1}: {download_url}")
                        download_success = await _run_download_in_tmux(ssh, session_name, base_cmd, "/tmp/splunk_install/splunkforwarder.tgz")
                    else:
                        logger.info(f"Attempting download with curl (no tmux) from URL #{url_index+1}: {download_url}")
                        download_result = await ssh.run(base_cmd)
                        download_success = download_result.returncode == 0
                    if download_success:
                        logger.info(f"Successfully downloaded from {download_url}")
                        break
                    else:
                        all_download_errors.append(f"curl error with URL #{url_index+1}")

                # Then wget
                if not download_success and has_wget:
                    base_cmd = f"cd /tmp/splunk_install && sudo wget -v -O splunkforwarder.tgz '{download_url}' 2>&1"
                    if has_tmux:
                        logger.info(f"Attempting download with wget in tmux from URL #{url_index+1}: {download_url}")
                        download_success = await _run_download_in_tmux(ssh, session_name, base_cmd, "/tmp/splunk_install/splunkforwarder.tgz")
                    else:
                        logger.info(f"Attempting download with wget (no tmux) from URL #{url_index+1}: {download_url}")
                        download_result = await ssh.run(base_cmd)
                        download_success = download_result.returncode == 0
                    if download_success:
                        logger.info(f"Successfully downloaded from {download_url}")
                        ...
                        all_download_errors.append(f"wget error with URL #{url_index+1}")
            ...
            await ssh.run(f"sudo mkdir -p {actual_extract_dir}/splunkforwarder/var")
            await ssh.run(f"sudo chown -R {user}:{group} {actual_extract_dir}/splunkforwarder/var")
            await ssh.run(f"sudo chmod -R 755 {actual_extract_dir}/splunkforwarder/var")
...
EOF'"""
            ...
            # Stop any existing Splunk processes first (clean start)
            logger.info("Stopping any existing Splunk processes...")
            await ssh.run(f"sudo -u {user} {actual_extract_dir}/splunkforwarder/bin/splunk stop 2>/dev/null || true")
            await ssh.run("sudo pkill -f splunk 2>/dev/null || true")
            await asyncio.sleep(3)
            
            # Clear any lock files before starting
            logger.info("Clearing any lock files...")
            await ssh.run(f"sudo rm -f {actual_extract_dir}/splunkforwarder/var/run/splunk/splunkd.pid 2>/dev/null || true")
            await ssh.run(f"sudo rm -f {actual_extract_dir}/splunkforwarder/var/run/splunk/splunkd.lock 2>/dev/null || true")
            ...
            if start_result.returncode != 0:
                ...
                if start_result.returncode != 0:
                    ...
                    log_check = await ssh.run(f"sudo tail -20 {actual_extract_dir}/splunkforwarder/var/log/splunk/splunkd.log 2>/dev/null || echo 'No log file found'")
                    logger.error(f"Splunk logs: {log_check.stdout}")
                    return {
                        "success": False,
                        "message": f"Failed to start Splunk UF: {start_result.stderr}. Check logs for details."
                    }
            
            # Wait for Splunk to fully start
            await asyncio.sleep(5)
            
            # Verify Splunk is actually running
            status_check = await ssh.run(f"sudo -u {user} {actual_extract_dir}/splunkforwarder/bin/splunk status")
            if "splunkd is running" not in status_check.stdout:
                logger.error("Splunk failed to start properly")
                log_check = await ssh.run(f"sudo tail -20 {actual_extract_dir}/splunkforwarder/var/log/splunk/splunkd.log 2>/dev/null || echo 'No log file found'")
                logger.error(f"Splunk logs: {log_check.stdout}")
                return {
                    "success": False,
                    "message": "Splunk failed to start properly. Check logs for details."
                    }
            ...
            # Verify and enable systemd service if created
            systemd_check = await ssh.run("test -f /etc/systemd/system/SplunkForwarder.service && echo 'exists' || echo 'not exists'")
            if "exists" in systemd_check.stdout:
                logger.info("Systemd service file created, enabling it...")
                await ssh.run("sudo systemctl daemon-reload")
                await ssh.run("sudo systemctl enable SplunkForwarder.service")
                logger.info("SplunkForwarder service enabled for boot")
            else:
                logger.info("Systemd service not found, checking for init.d script...")
                initd_check = await ssh.run("test -f /etc/init.d/splunk && echo 'exists' || echo 'not exists'")
                if "exists" in initd_check.stdout:
                    await ssh.run("sudo chkconfig splunk on 2>/dev/null || sudo update-rc.d splunk defaults 2>/dev/null || true")
                    logger.info("Init.d script enabled for boot")
            ...
            if "splunkd is running" not in status_result.stdout:
                logger.error("Splunk is not running after installation")
                # Try to get more diagnostic information
                log_check = await ssh.run(f"sudo tail -20 {actual_extract_dir}/splunkforwarder/var/log/splunk/splunkd.log 2>/dev/null || echo 'No log file found'")
                logger.error(f"Splunk logs: {log_check.stdout}")
                
                # Try to start it one more time
                logger.info("Attempting final start attempt...")
                final_start = await ssh.run(f"sudo -u {user} {actual_extract_dir}/splunkforwarder/bin/splunk start --accept-license --no-prompt --answer-yes")
                await asyncio.sleep(5)
                
                final_status = await ssh.run(status_cmd)
                if "splunkd is running" not in final_status.stdout:
                    return {
                        "success": False,
                        "message": "Splunk failed to start after installation. Check logs for details."
                    }
                else:
                    logger.info("Splunk started successfully on final attempt")
