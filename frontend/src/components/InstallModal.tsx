...
const InstallModal: React.FC<InstallModalProps> = ({ visible, onClose, onSuccess }) => {
  ...
  const INSTALLATION_TYPES = [
    { value: 'splunk_cm', label: 'Splunk CM' },
    { value: 'splunk_deployer', label: 'Splunk Deployer' },
    { value: 'splunk_license_master', label: 'Splunk License Master' },
    { value: 'splunk_monitoring_console', label: 'Splunk Monitoring Console' },
    { value: 'splunk_deployment_server', label: 'Splunk Deployment Server' },
    { value: 'splunk_search_head', label: 'Splunk Search Head' },
    { value: 'splunk_indexer', label: 'Splunk Indexer' },
    { value: 'splunk_hf', label: 'Splunk HF' },
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (installType === 'splunk_uf') {
        ...
        if (values.cluster_name && values.cluster_role) {
          parameters.cluster_name = values.cluster_name;
          parameters.cluster_role = values.cluster_role;
        }
      } else if (installType === 'splunk_enterprise') {
        parameters.admin_password = values.admin_password;
        parameters.user = values.user || 'splunk'; // Backend requires 'user' parameter
  ...
  // Handle view job progress
  const handleViewJobProgress = () => {
    // Navigate to job history page with the specific job ID
    navigate(`/jobs?job_id=${jobId}`);
    onClose();
  };

  // Handle auto-redirect to job history
  const handleAutoRedirect = () => {
    // Automatically redirect to job history page after a short delay
    setTimeout(() => {
      navigate(`/jobs?job_id=${jobId}`);
      onClose();
  ...
  // Auto-redirect after successful job submission
  useEffect(() => {
    if (jobId && currentStep === 3) {
      handleAutoRedirect();
    }
  }, [jobId, currentStep]);
          ...
          {installType === 'splunk_enterprise' && (
            <>
              ...
              <Form.Item
                ...
                label="Run User"
                initialValue="splunk"
                rules={[{ required: true, message: 'Please enter run user' }]}
              >
      ...
      case 3:
        ...
        return (
          <Result
            ...
            extra={[
              <Button key="view" type="primary" size="large" onClick={handleViewJobProgress} icon={<EyeOutlined />}>
                View Job Progress
              </Button>,
